defmodule Interpolation do
  ...
  ## Examples
      ...
      iex> Interpolation.to_interpolatable(msgid)
      ["Hello ", :name, ", you have ", :count, " unread messages"]
      iex> Interpolation.to_interpolatable("Empties %{} stay empty")
      ["Empties %{} stay empty"]
  """
  ...
  ## Examples
      iex> msgid = "Hello %{name}, you have %{count} unread messages"
      iex> interpolatable = Interpolation.to_interpolatable(msgid)
      iex> good_bindings = %{name: "José", count: 3}
      iex> Interpolation.interpolate(interpolatable, good_bindings)
      {:ok, "Hello José, you have 3 unread messages"}
      iex> Interpolation.interpolate(interpolatable, %{name: "José"})
  ...
  returned.
  ## Examples
      iex> Interpolation.keys("Hey %{name}, I'm %{other_name}")
      [:name, :other_name]
      iex> Interpolation.keys(["Hello ", :name, "!"])
      [:name]
      iex> Interpolation.keys(["Hello ", :name, "! Goodbye", :name])
      [:name]
  """
